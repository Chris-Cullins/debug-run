...
const ACTIONABLE_HINTS: Record<string, string> = {
  ...
  "SocketException:10061": "Connection refused - check if the target service is running and the port is correct",
  ...
  // SQL Server errors
  "SqlException:18456": "SQL Server login failed - verify database credentials and user permissions",
...
function findInnerException(variable: VariableValue): VariableValue | undefined {
  if (typeof variable.value === "object" && variable.value !== null) {
    ...
    if (inner && inner.type) {
      ...
      if (inner.type === "null" || inner.type.toLowerCase() === "object" && inner.value === null) {
        return undefined;
      }
...
export async function flattenExceptionChain(
  ...
  try {
    ...
    while (depth < maxDepth) {
      ...
      const source = sourceResult && sourceResult !== "null" ? cleanString(sourceResult) : undefined;
...
async function extractThrowSite(
  ...
  try {
    ...
    const targetSite = await safeEvaluate(
      client, 
      frameId, 
      `${exceptionExpr}.TargetSite?.Name`
    );
...
async function extractExceptionData(
  ...
  if (exceptionType.includes("ArgumentException") || exceptionType.includes("ArgumentNullException")) {
